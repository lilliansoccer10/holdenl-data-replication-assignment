...
dataII <- data %>%  
  group_by(Tree_sp, Forest_type) %>%  
  reframe(Total=CT_trap_nights, Min=min(CT_trap_nights), Max=max(CT_trap_nights), Visitor_species_type=Body_size, Total_visits=Num_visits, Mean_trap_nights=mean(CT_trap_nights), Sd_trap_nights=sd(CT_trap_nights))
dataII
```
