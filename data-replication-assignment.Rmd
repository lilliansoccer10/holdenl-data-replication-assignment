...
```{r}
data <- read_csv("/Users/lilyholden/Documents/GitHub/holdenl-data-replication-assignment/Visits_seedpredators_CT.csv", show_col_types = TRUE)
data
```
...
dataII <- data %>%  
  group_by(Tree_sp) %>%  
  summarize(CT_trap_nights_mean=mean(CT_trap_nights), sd_CT_trap_nights=sd(CT_trap_nights_mean), Visitor_species_type=Body_size, Num_visits)
dataII
```
