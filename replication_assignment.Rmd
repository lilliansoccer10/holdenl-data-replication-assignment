...
```{r}
glmmI <- glmer(data=glmdata, Vertebrate_pred~Seed_size + Forest_type + Seed_size + (1 | Tree_sp), family = poisson(link="log"))
summary(glmmI)
```

```{r eval=FALSE, warning=FALSE, include=FALSE}
glmmII <- glmer(Invertebrate_prop~Forest_type + Seed_size + (1 | Tree_sp), data=prop, family="binomial")
summary(glmmII)
```

```{r}
glmmIII <- glmer(data=glmdata, Invertebrate_pred~Seed_size + Forest_type + Seed_size + (1 | Tree_sp), family = poisson(link="log"))
summary(glmmIII)
```
```{r warning=FALSE}
glmmIV <- glmer(Invertebrate_prop~Forest_type + Seed_size + (1 | Tree_sp), data=newdata, family="binomial")
summary(glmmIV)
```

```{r}
glmmV <- glmer(data=glmdata, Removed~Seed_size + Forest_type + Seed_size + (1 | Tree_sp), family = poisson(link="log"))
summary(glmmV)
```