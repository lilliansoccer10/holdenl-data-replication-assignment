...
#### Piping to Find the Maximum Number of Trap Nights of Seed Predators in a Night per Tree Species
```{r}
dataIIII <- 
  data %>%
  group_by(Tree_sp) %>%
...
#### Piping to Find the Minimum Number of Trap Nights of Seed Predators per Tree Species
```{r}
dataV <- 
  data %>%
  group_by(Tree_sp) %>%
...
#### Piping to Find the Total Trap Nights of Seed Predator Visits per Tree Species
```{r}
dataVII <- 
  data %>%
  group_by(Tree_sp) %>%
...
#### Finding the Mean and Standard Deviation of Seed Predator Visits per Trap Night
```{r}
dataVIII <- merge(dataVII, dataVI) #merge the data required
dataVIII
```

<br>

#### Piping to Find the Mean and Standard Deviation of Seed Predator Visits per Trap Night
...
The marginal R2 for the mixed model is the same as published in the paper, however the conditional is not for some reason. 
