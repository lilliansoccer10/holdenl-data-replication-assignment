...
### Load in the Data
```{r}
glmdata <- read_csv("/Users/lilyholden/Documents/GitHub/holdenl-data-replication-assignment/Overall_visits.csv", show_col_types = FALSE)
head(glmdata)
```

```{r}
newglmmdata <- merge(glmdata, data)
newglmmdata
```
...
```{r warning=FALSE}
glmmI <- glmer(Num_visits~Forest_type + (1 | Tree_sp), data=newglmmdata, family = "poisson") 
summary(glmmI)
```
