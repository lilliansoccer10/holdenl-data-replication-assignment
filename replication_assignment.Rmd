...
```{r}
gsf <- glmer(Vertebrate_prop~ Forest_type*Seed_size + (1 | Tree_sp), data = newdata  ,family = "binomial", control = glmerControl(optimizer = "bobyqa"))
```
lmer <- glmer()
```{r}
lme <- glmer(Vertebrate_prop ~ Forest_type*Seed_size + (1 | Tree_sp), family = "poisson", data=newdata)
...
```{r}
lme <- glmer(data=glmdata, Invertebrate_pred~Forest_type*Seed_size + (1 | Tree_sp), family = "poisson")
summary(lme, corr=FALSE)
```

```{r}
lme <- glmer(data=glmdata, Removed~Forest_type*Seed_size + (1 | Tree_sp), family = "poisson")
summary(lme, corr=FALSE)
```